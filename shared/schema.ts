...
export const inputBasisEnum = pgEnum("input_basis", [
  "PER_UNIT_OUTPUT",
  "PER_UNIT_INPUT",
]);
...
export const carryForwardStatusEnum = pgEnum("carry_forward_status", [
  "PENDING",
  "APPROVED",
  "REJECTED",
]);
...
export const products = pgTable("products", {
  ...
  isIngredientOnly: boolean("is_ingredient_only").notNull().default(false),
  active: boolean("active").notNull().default(true),
...
export const formulas = pgTable("formulas", {
  ...
  inputBasis: inputBasisEnum("input_basis")
    .notNull()
    .default("PER_UNIT_OUTPUT"),
  active: boolean("active").notNull().default(true),
...
export const conversionFormulas = pgTable("conversion_formulas", {
  ...
  inputProductId: integer("input_product_id")
    .notNull()
    .references(() => products.id),
  ratioNumerator: decimal("ratio_numerator", {
    precision: 10,
    scale: 4,
  }).notNull(),
  ratioDenominator: decimal("ratio_denominator", {
    precision: 10,
    scale: 4,
  }).notNull(),
  notes: text("notes"),
...
export const insertConversionFormulaSchema = createInsertSchema(
  conversionFormulas,
).omit({ id: true });
export type InsertConversionFormula = z.infer<
  typeof insertConversionFormulaSchema
>;
...
export const insertBlendComponentSchema = createInsertSchema(
  blendComponents,
).omit({ id: true });
...
export const yieldTolerances = pgTable("yield_tolerances", {
  ...
  productId: integer("product_id")
    .notNull()
    .references(() => products.id),
  warnPercent: decimal("warn_percent", { precision: 5, scale: 2 })
    .notNull()
    .default("5"),
  criticalPercent: decimal("critical_percent", { precision: 5, scale: 2 })
    .notNull()
    .default("10"),
});
...
export const insertDailyIntakeSchema = createInsertSchema(dailyIntakes).omit({
  id: true,
  createdAt: true,
  reviewedAt: true,
...
export const productionBatches = pgTable("production_batches", {
  ...
  systemCalculatedRemaining: decimal("system_calculated_remaining", {
    precision: 12,
    scale: 4,
  }),
...
export const insertProductionLineItemSchema = createInsertSchema(
  productionLineItems,
).omit({
  id: true,
  createdAt: true,
...
export type InsertProductionLineItem = z.infer<
  typeof insertProductionLineItemSchema
>;
...
export const insertPackoutSchema = createInsertSchema(packouts).omit({
  id: true,
  createdAt: true,
  reviewedAt: true,
  reviewedByUserId: true,
...
export const blendActualUsage = pgTable("blend_actual_usage", {
  ...
  lineItemId: integer("line_item_id")
    .notNull()
    .references(() => productionLineItems.id, { onDelete: "cascade" }),
  componentProductId: integer("component_product_id")
    .notNull()
    .references(() => products.id),
...
export const insertBlendActualUsageSchema = createInsertSchema(
  blendActualUsage,
).omit({ id: true });
export type InsertBlendActualUsage = z.infer<
  typeof insertBlendActualUsageSchema
>;
...
export const changeRequests = pgTable("change_requests", {
  ...
  reviewedByAdminUserId: integer("reviewed_by_admin_user_id").references(
    () => users.id,
  ),
  reviewedAt: timestamp("reviewed_at"),
...
export const insertChangeRequestSchema = createInsertSchema(
  changeRequests,
).omit({
  id: true,
  requestedAt: true,
...
export const lossThresholds = pgTable("loss_thresholds", {
  ...
  minLossPercent: decimal("min_loss_percent", { precision: 5, scale: 2 })
    .notNull()
    .default("0"),
  maxLossPercent: decimal("max_loss_percent", { precision: 5, scale: 2 })
    .notNull()
    .default("5"),
...
export const insertLossThresholdSchema = createInsertSchema(
  lossThresholds,
).omit({ id: true });
...
export const carryForwardRequests = pgTable("carry_forward_requests", {
  ...
  systemCalculatedAmount: decimal("system_calculated_amount", {
    precision: 12,
    scale: 4,
  }),
...
export const insertCarryForwardRequestSchema = createInsertSchema(
...
export type InsertCarryForwardRequest = z.infer<
  typeof insertCarryForwardRequestSchema
>;
...
export const insertDailyLockSchema = createInsertSchema(dailyLocks).omit({
  id: true,
  lockedAt: true,
});
